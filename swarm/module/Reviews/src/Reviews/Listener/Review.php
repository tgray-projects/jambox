    ...
    public function processReview(Event $event)
    {
        ...
        try {
            ...
            if ($data['updateFromChange']) {
                ...
                $doRequest = function (
                    ...
                    try {
                        ...
                        // if we have post parameters, ensure we treat the request as a POST
                        if (!empty($postParams)) {
                            $client->setMethod(Request::METHOD_POST);

                            // parse parameters based on their format (GET or JSON)
                            if ($postFormat == 'GET') {
                                parse_str($postParams, $postParams);
                ...
                // @todo accommodate separate (potentially conflicting) results from each project
                foreach ($currentProjects as $projectId => $branches) {
                    ...
                    if ($project && $project->getTests('enabled') && $project->getTests('url')) {
                        ...
                        $doRequest(
                            ...
                            $url,
                            'automated tests',
                            str_replace($search, $replace, trim($project->getTests('postParams'))),
            ...
            if ($data['isAdd']) {
                ...
                $groups   = $projects ? Group::getCachedData($p4Admin) : null;
