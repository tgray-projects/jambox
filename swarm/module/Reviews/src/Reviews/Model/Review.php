...
class Review extends KeyRecord
{
    ...
    public function updateFromChange($change, $unapproveModified = true)
    {
        ...
        try {
            ...
            if ($change->isPending() && $changesDiffer) {
                $result = $p4->run('unshelve', array('-s', $change->getId(), '-c', $shelf->getId()));
    ...
    public function commit(array $options = array(), Connection $p4 = null)
    {
        ...
        try {
            ...
            $result = $p4->run('unshelve', array('-s', $this->getId(), '-c', $commit->getId()));
            ...
            // delete the commit change we created; its no longer needed
    ...
    /**
     * Get API token associated with this review.
     ...
     *
     * @return  array   the token for this review
     */
    public function getToken()
    {
        return $this->getRawValue('token');
    }
    ...
    {
        return (array) $this->getRawValue('testDetails');
    }
    ...
    public function save()
    {
        ...
        $this->set('token', $this->get('token') ?: strtoupper(new Uuid));
    ...
    protected function guessStreamFromChange($change)
    {
        ...
        // grab the depot and first path component off the first file and see if its a stream
        preg_match('#^(?P<stream>//(?P<depot>[^/]+)/[^/]+)/#', $file, $matches);
        if (isset($matches['depot'])) {
            $depot = Depot::fetch($matches['depot'], $p4);
