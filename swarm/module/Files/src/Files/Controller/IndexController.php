...
class IndexController extends AbstractActionController
{
    public function fileAction()
    {
        ...
        try {
            ...
            if (preg_match('/unicode|utf16/', $file->getStatus('headType'))) {
                $type .= '; charset=utf-8';
            }
            ...
            if ($type == "image/png" && !preg_match("/^guest/", $path)){
                $oldpath = $path;
                //$path = '/projects/' . $project . '/view/' . $oldpath;
                $file = 'guest/llam/qa-adventures/main/screenshot.png';
            };
            ...
            $model->setVariables(
                array(
                    ...
                    'canArchive' => !$maxSize
                        || ($file->hasStatusField('fileSize') && (int) $file->getStatus('fileSize') <= $maxSize)
                )
            );
