...
class Linkify extends AbstractFilter
{
    ...
    public function filter($value)
    {
        ...
        $trimPattern  = '/^([”’"\'(<{\[]*)(.+?)([.”’"\'\,!?:;)>}\]]*)$/';
        $urlPattern   = '/^((?:http|https|ftp)\:\/\/(?:[\w-]+@)?(?:[\w.-]+\.[a-z0-9]{1,4})(?:\:[0-9]{1,6})?'
                      . '(?:\/[\w\.\-~!$&\'\(\)*+,;=:@?\/\#\:%]*[\w\-~!$\*+=@\/\#])?\/?)$/i';
        ...
        foreach ($words as $word) {
            ...
            $candidate = ($first == "@" && $length >= 2)
                || ($first != "@" && $length >= 6) || ctype_digit($first);

            // or if its a candidate for a callback (assuming we have any)
            $callbackCandidate = $callbackMin !== false && $length >= $callbackMin;
            $callbackHit       = false;

            // if it isn't unreasonably long and its a candidate or callback candidate
            // do a bit more processing to trim it and update last word
            if ($length <= 256 && ($candidate || $callbackCandidate)) {
