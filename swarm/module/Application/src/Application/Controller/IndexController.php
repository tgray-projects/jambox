...
use Application\Support\SystemData;
use P4\Spec\Job;
use P4\File\File;
use P4\Spec\Depot;
use P4\Spec\Change;
...
class IndexController extends AbstractActionController
{
    ...
    {
        // only admins or super users can download the support archive
        $services = $this->getServiceLocator();
        ...
        $dir = tempnam(sys_get_temp_dir(), 'swarm-archive-');
        if ($dir === false) {
            throw new \RuntimeException('Could not create a temporary directory for the support archive.');
        }
        ...
        // create symlinks for license and log, which allows us to archive them without copying
        $source = DATA_PATH . '/swarm-license';
        if (file_exists($source) && is_readable($source)) {
            symlink($source, $dir . '/swarm-license');
        }

        $source = DATA_PATH . '/log';
        if (file_exists($source) && is_readable($source)) {
            symlink($source, $dir . '/log');
        }

        // use output buffering to grab phpinfo, and output to file
        ob_start();
        phpinfo();
        file_put_contents($dir . '/phpinfo.html', ob_get_contents());
        ob_end_clean();

        // write results of 'p4 info' to  file
        file_put_contents($dir . '/p4info.txt', var_export($p4->getInfo(), true));

        // collect system information and dump to a file
        $data = new SystemData($services->get('config'));
        file_put_contents($dir . '/systeminfo.txt', var_export(array_reverse($data->getData()), true));

        // build the archive
        $archive = new CommandLineCompression;
        $error   = false;
        try {
            // compress will throw an exception if it can't find a suitable method
            if (!$archive->compress($dir)) {
                $error = 'Unable to create the support archive. Please check your swarm log.';
            }
        ...
        if ($error !== false) {
            return $view->setVariable('error', $error);
        }

        if ($archive->getArchive()) {
            // send the archive - since we used a tmpfile, it will be removed after the request completes
            $response = new \Zend\Http\Response\Stream;
            $headers  = new \Zend\Http\Headers;
            $headers->addHeaderLine('Content-type', 'application/octet-stream');
            $headers->addHeaderLine(
                'Content-Disposition',
                'attachment; filename=swarm-support-archive-' . date('Y-m-d_H:i:s') . '.' . $archive->getMethod(),
                true
            );

            $response->setHeaders($headers);
            $response->setStream(fopen($archive->getArchive(), 'r'));
            return $response;
        }
    }
