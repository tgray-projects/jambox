        ...
        foreach ($checks as $check => $value) {
            ...
            switch ($check) {
                ...
                case 'member':
                    ...
                    throw new \InvalidArgumentException("The member test requires a project or a group ID as input.");
                    break;
                case 'owner':
                    if (!$value instanceof Project) {
                        throw new \InvalidArgumentException("The owner access test requires a project as input.");
                    }
                    if (!in_array($p4User->getUser(), $value->getOwners())) {
                        throw new ForbiddenException("This operation is limited to project owners.");
                    }
                    break;
                case 'projectAddAllowed':
                    ...
                    //  - is also an admin if $config['security']['add_project_admin_only'] is set to true
                    //  - is also a member of at least one of the groups specified in
                    //    $config['security']['add_project_groups'] if this value is set and not empty
                    $config = $this->serviceLocator->get('config');

                    // check admin restriction if required
                    ...
                    // check project groups restriction if specified
