...
swarm.browse = {
    ...
    openPath: function(path, isHistory, isFile) {
        ...
        $('.tab-content').trigger('contentchanged');
    },
    ...
    initHistory: function() {
        ...
        // load change history if it wasn't sent with the page and the history tab is active
        if (!target.children().length && $('.browse-tabs .active a[href="#commits"]').length) {
            swarm.changes.load(path, target, { user: userInput.val(), range: rangeInput.val() });
        }

        // load change history when tab is shown and history not yet loaded
        // clear existing listener first to avoid connecting multiple times
        $(document).off('shown.history');
        $(document).on('shown.history', '.browse-tabs a[href="#commits"]', function(e) {
            if (!target.children().length) {
                swarm.changes.load(path, target, { user: userInput.val(), range: rangeInput.val() });
            }
        });

        // load more history when user scrolls down
        // clear existing listener first to avoid connecting multiple times
        ...
        $.ajax({
            ...
            success:    function(data) {
                ...
                $('.view-text ol li .annotation').each(function(){
                    ...
                    $this.html($.templates(
                        '<a href="{{url:"/changes"}}/{{urlc:id}}"><span class="text">{{>change.user}}</span></a>'
                    ).render({id: id, change: change}));

                    // if the same change appears consecutively, diminish it
                    $this.addClass(repeat ? 'repeat' : '');

                    // prepare popover (tooltip) contents
                    ...
                    // activate timeago elements when they first appear
                    $this.find('a').one('shown', function(){
    ...
    getShortLink: function(button) {
        ...
        $.post(
            ...
            function(data) {
                ...
                // add a class to the popover so we can target it via css
                var tip = button.data('popover').tip();
                tip.addClass('short-link-popover');
