...
swarm.reviews = {
    ...
    toggleFilter: function(button, applyFilter) {
        ...
        if (!$(button).closest('.dropdown-menu').length) {
            ...
            $(button).closest('.btn-group').find('.text').html($(button).html());
        }
    ...
    getFilters: function(tabPane) {
        ...
        $(tabPane).find('.btn-filter.active').each(function(){
            var button      = $(this),
                ...
                filterKey   = group.data('filterKey'),
...
swarm.review = {
    ...
    initReadUnread: function() {
        ...
        $('.change-files').on('click', '.btn-file-read', function(e){
            ...
            $.ajax({
                type:  "POST",
                url:   '/reviews/' + review.id + '/v' + encodeURIComponent(change.rev)
    ...
    initSlider: function() {
        ...
        $.each(review.versions, function(index) {
            this.rev      = index + 1;
            this.selected = parseInt(this.change, 10) === parseInt(change.id, 10)
                && this.rev === parseInt(change.rev, 10);
            data.push(this);
        });
    ...
    updateTestStatus: function(){
        var wrapper = $('.review-wrapper'),
            ...
            testUrl = review.testDetails.url ? encodeURI(review.testDetails.url) : '',
            endTime = review.testDetails.endTime;
    ...
    add: function(button) {
        button      = $(button);
        var change  = button.closest('.change-wrapper').data('change');

        // disable button while talking to the server.
        swarm.form.disableButton(button);

        $.post('/reviews/add', {change: change.id}, function(response) {
            if (response.id) {
