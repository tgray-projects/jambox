...
swarm.jobs = {
    ...
    init: function(attachListeners) {
        ...
        // if authenticated, and a project is present, add the job add button for this project
        // if not authenticated, ensure the page reloads when the user logs in
        if ($('body').hasClass('authenticated')) {
            var projectId = $('body').attr('data-project');
            var toolbar   = $('.jobs-toolbar');

            if (projectId && toolbar) {
                var jobButton = '<button type="button" class="btn project-add-job"><i class="icon-plus">'
                    + '</i> Add Job</button>';

                $('.jobs-toolbar .btn-select-columns').after(jobButton);
                $('.project-add-job').on('click', function() {
                    var url = swarm.url('/projects/' + projectId + '/job/add');
                    swarm.jobs.openEditDialog({title: 'Add', url: url});
                });
            }
        } else {
    ...
    openSelectColumns: function(element) {
        ...
        element.popover({
            ...
            placement: 'bottom',
            container: 'body',
            template:  '<div class="popover popover-job-fields"><div class="arrow"></div><div class="popover-content"></div></div>'
        }).popover('show');
    ...
    openEditDialog: function(data) {
        ...
        $('.modal.job-edit').remove();

        var modal = $(
            '<div class="modal hide fade job-edit" tabindex="-1" role="dialog" aria-labelledby="edit-title" aria-hidden="true">'
                +    '<div class="modal-header">'
                +        '<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>'
                +        '<h3 id="edit-title">' + data.title + ' Job</h3>'
                +    '</div>'
                +    '<div class="modal-body">'
                +        '<div class="jobs-form">'
                +           '<div class="loading animate muted pad3">Loading...</div>'
                +        '</div>'
                +    '</div>'
                +    '<div class="modal-footer">'
                +        '<button type="button" class="btn btn-primary btn-submit" disabled>Save</button>'
        ...
        // load dialog content
        $.ajax(swarm.url(data.url), {
            dataType:   'html',
            data:       {format: 'partial'},
            success:    function(data) {
                swarm.modal.show(modal);
                var browser = modal.find('.jobs-form');
                browser.html(data);
        ...
        modal.find('.btn.btn-submit').on('click', function(e){
            // read the current active button and update hidden inputs accordingly
            $.each($('input[type="hidden"]'), function(index, obj) {
                var name = $(obj).attr('name');
                var className = 'button.job-' + name + '.active';
            ...
            e.preventDefault();
            swarm.form.post(swarm.url(data.url), modal.find('form'));
        });
    },
